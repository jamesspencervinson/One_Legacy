function obj=log_ll(para, wait_cost, beta, T, N, n, ...
                    alloc_vec, donor_index, tran_matrix, time_index, state_index, cross_clamp, donor_info)
...
costs = [wait_cost; para(7:9)];


for k=1:n
    j = donor_info(k,4);
    
    c = donor_info(k,2:3)*para(10:11);

%     c = para(6+j);
    wait_cost = costs(j);
    if j==1
        para_temp=para_vec{j,1} + [c;c;c;0;0;0];
    elseif j==4
        para_temp=para_vec{j,1} + c;
    else
        para_temp=para_vec{j,1} + [c;c;0];
    end
...
%     tran_matrix_temp=tran_matrix{j,1};    
    ...
    V_vec{k,1}=V_temp;
    choice_prob_vec{k,1}=choice_prob_temp;
...
for i=1:N
    ...
    choice_prob_temp=choice_prob_vec{donor_index(i),1};
