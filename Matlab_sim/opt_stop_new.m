...
for k = 1:T
    ...
    prob_HR = [1-p_HR p_HR];
    prob_LU = [(1-p_LU)^2 2*(1-p_LU)*p_LU p_LU^2];
    prob_LI = [1-p_LI p_LI];
    ...
    for i=1:size(alloc_vec_c1,1)
        for j=1:size(alloc_vec_c1,1)
            ...
            diff = a - b;
            if all(diff >= 0)
                tran_matrix_c1(i, j)=prob_HR(diff(1)+1)*prob_LU(diff(2)+1)*prob_LI(diff(3)+1);
            else
    ...
    for i=1:size(alloc_vec_c2,1)
        for j=1:size(alloc_vec_c2,1)
            ...
            diff = a - b;
            if all(diff >= 0)
                tran_matrix_c2(i, j)=prob_HR(diff(1)+1)*prob_LI(diff(2)+1);
            else
