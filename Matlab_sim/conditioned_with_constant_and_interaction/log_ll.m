...
para_vec=cell(4,1);
para_vec{1,1}=para;
para_vec{2,1}=[para(1); para(3); para(5); para(7)];
para_vec{3,1}=[para(2); para(3); para(6); para(7)];
para_vec{4,1}=[para(3); para(7)];
...
alloc_vec{1,1} = [temp interaction o];

temp = alloc_vec{2,1};
o = ones(4,1);
interaction = [temp(:,1).*temp(:,2)];
alloc_vec{2,1} = [temp interaction o];

temp = alloc_vec{3,1};
o = ones(6,1);
interaction = [temp(:,1).*temp(:,2)];
alloc_vec{3,1} = [temp interaction o];

alloc_vec{4,1} = [alloc_vec{4,1} ones(2,1)];
